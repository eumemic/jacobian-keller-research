...
#       polynomial b_0 FORCES h | a_1 (kappa != 0).  For kappa = 0, Q_2 forces it
#       only on the b_1 = c h, c != 0 branch; the inherited b_1 = 0 branch remains
#       open.  On the proven branches G = h^[-1] M then has every divisibility derived.
#
...
#       This verifier records local slack for diff-1/diff-2 and re-verifies cap-D=2
#       emptiness (plus optional D=3,4 msolve corroboration).  The current sector
#       status is cross-referenced at the end; its arbitrary-degree certificates live
#       in band3-sectors.md / verify_band3_sectors.py and are not reproduced here.
#
...
#                  on kappa != 0 and kappa = 0, b_1 != 0 branches, RESIDUAL 2 diff-3
#                  closure + diff-1/2 slack + cap-D=2 emptiness, and RESIDUAL 3
#                  gauge-obstruction + positive-cascade-permits.
...
# A successful run with optional omissions ends:
#   ALL EXECUTED SHIFTED POWER RESIDUAL CHECKS PASSED; OPTIONAL CHECKS SKIPPED
...
def _serialize_msolve_polynomial(e, allc):
    """Clear coefficient-only denominators and emit one safe msolve polynomial."""
    num, den = sp.fraction(sp.together(sp.sympify(e)))
    variables = set(allc)
    if den.free_symbols & variables:
...
def msolve_unit(eqs, allc, tag, timeout=400):
    """Return True iff msolve emits its exact whole stdout unit record '[1]:'."""
    varstr = ", ".join(str(v) for v in allc)
    body = ",\n".join(_serialize_msolve_polynomial(e, allc) for e in eqs)
    ms = f"{varstr}\n0\n{body}\n"
    if '/' in ms or '**' in ms:
        raise ValueError("msolve input contains unsafe '/' or '**' serialization")
    with tempfile.TemporaryDirectory(prefix=f"spr_{tag}_") as tmp:
        path = os.path.join(tmp, 'input.ms')
        with open(path, 'w', encoding='utf-8') as fh:
            fh.write(ms)
        t = time.time()
        r = subprocess.run(['msolve', '-g', '2', '-f', path],
                           capture_output=True, text=True, timeout=timeout, check=True)
        unit = _parse_msolve_stdout_unit(r.stdout)
        dt = time.time() - t
...
check("msolve stdout parser accepts only the canonical whole reduced-GB unit record",
...
def _serialization_rejected(e, allc):
    try:
        _serialize_msolve_polynomial(e, allc)
    except ValueError:
        return True
    return False


_ser_x = sp.symbols('serialization_x')
check("msolve serialization accepts rational constants by clearing their common denominator",
      _serialize_msolve_polynomial(sp.Rational(2, 3) * _ser_x**2 - sp.Rational(5, 7),
                                   [_ser_x]) == '14*serialization_x^2-15')
check("msolve serialization rejects variable-dependent denominators",
      _serialization_rejected(1 / (_ser_x + 1), [_ser_x]))
check("msolve serialization rejects unsafe residual slash/power syntax",
      _serialization_rejected(sp.Symbol('unsafe/name') + _ser_x, [_ser_x]))
...
# (1d) kappa = 0 branch: b_2 = 0, Q_4 => b_1 = c h, and Q_3 gives
#      h h^[1] | a_2.  Recompute Q_2 independently in the crossed product.  Every
#      term except the (a_1,b_1) crossing is divisible by h h^[1]:
#          Q_2 = c(h^[1]a_1 - h a_1^[1]) mod h h^[1].
#      At the node E=rho this reads 0 = c a_1(rho), so h|a_1 only for c != 0.
cc = sp.symbols('cc')
hh = E - rho; h1, h2 = sh(hh, 1), sh(hh, 2)
a3 = prodsh(hh, 3)
g2, _ = gen('kg', 2); a2k = sp.expand(hh * h1 * g2)
a1f, _ = gen('ka1', 3); a0k, _ = gen('ka0', 2)
b0k, _ = gen('kb0', 3)
bm1k, _ = gen('kbm1', 3); bm1k = sp.expand(E * bm1k)
Xk = {3: a3, 2: a2k, 1: a1f, 0: a0k}
Dk = {3: sp.Integer(0), 2: sp.Integer(0), 1: sp.expand(cc * hh), 0: b0k, -1: bm1k}
q2_crossed = sp.expand(cc * (h1 * a1f - hh * sh(a1f, 1)))
q2_remainder = sp.expand(Qm(Xk, Dk, 2) - q2_crossed)
check("RESIDUAL 1 (kappa=0): exact Q_2 crossing modulo h h^[1] "
      "(b_2=0, b_1=c h, a_2=h h^[1]g)",
      divides(hh * h1, q2_remainder))
node_a1, cinv = sp.symbols('node_a1 cinv')
node_nz_ideal = sp.groebner([cc * node_a1, cinv * cc - 1], node_a1, cinv, cc)
check("RESIDUAL 1 (kappa=0, c!=0): Q_2 node value forces a_1(rho)=0, hence h | a_1",
      sp.expand(q2_crossed.subs(E, rho) - cc * a1f.subs(E, rho)) == 0
      and sp.expand(h1.subs(E, rho)) == 1
      and node_nz_ideal.reduce(node_a1)[1] == 0)
check("RESIDUAL 1 (kappa=0, c=0): this Q_2 route gives no node constraint; "
      "the b_1=0 branch is closed independently by the Nonpositive-D Exclusion Theorem",
      sp.expand(q2_crossed.subs(cc, 0)) == 0)

# (1e) chain closes on the branches where h | a_1 is derived (kappa != 0, or
#      kappa = 0 with c != 0): the closed-form central factorization G = h^[-1] M
#      holds with no imported divisibility on those branches.
...
check("RESIDUAL 1 (proven branches): G = h^[-1] M once h|a_1 is derived "
      "(kappa!=0; or kappa=0, b_1!=0)",
...
# cube-separated) is CLOSED at arbitrary degree.  The direct Q_4 divisibility route
# breaks for diff-1 and diff-2; newer sector certificates restore diff-1 only (see §4).
...
# ===========================================================================
print("   Gap 2 band-3 shifted-cube status (current cross-reference; audit 2026-07-26):")
print("     This verifier proves its displayed residual identities and bounded cap-D=2 checks;")
print("     arbitrary-degree sector certificates are in band3-sectors.md / verify_band3_sectors.py")
print("     and shifted-cube-completion.md.")
print("     diff-1: FULLY CLOSED at arbitrary degree in all three branches --")
print("       kappa != 0: this file's Q_3 forcing + band3-sectors.md radical certificate;")
print("       kappa = 0, b_1 != 0: this file's Q_2-cV congruence route;")
print("       kappa = 0, b_1 = 0: Nonpositive-D Exclusion Theorem of")
print("       shifted-cube-completion.md. The direct residual/congruence route here stays")
print("       silent at kappa=0,b_1=0, but that theorem closes the branch independently.")
print("     diff-2: kappa = 0, b_1 = 0 CLOSED by that theorem; kappa = 0, b_1 != 0")
print("       closed by direct cascade; kappa != 0 branch remains OPEN at arbitrary degree")
print("       (clean restoration refuted; cap-D=2 emptiness here is corroboration only).")
print("     diff-3: CLOSED arbitrary degree here (2-separation suffices).")
print("     A*-band3 general kappa_2 != 0: OPEN. The Nonpositive-D Exclusion Theorem")
print("       closes only kappa_2=0,b_1=0; the existing kappa_2=0 tame witness has")
print("       positive band-one D (b_1 != 0), so there is no contradiction with that")
print("       theorem. Composite tame escape + negative tail OPEN.")
...
if n_pass == n_tot:
    if SKIPS:
        print("ALL EXECUTED SHIFTED POWER RESIDUAL CHECKS PASSED; OPTIONAL CHECKS SKIPPED")
    else:
