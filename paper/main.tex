...
A \emph{Keller map} is a polynomial map with nonzero constant Jacobian determinant, terminology originating with Keller's formulation of the Jacobian problem \cite{Keller39}.  Its \emph{geometric degree} is the degree of the induced extension of rational function fields, equivalently the cardinality of a general geometric fiber.  We work over $\C$, although the displayed identities are defined over $\Q$.

The claims below are deliberately limited to the construction, its degree, and the two explicitly certified monodromy groups.  Contemporaneous public work also reported constructions in every generic fiber degree at least three \cite{Gallagher26}; we record that overlap neutrally and make no claim here about exact priority or novelty.  The accompanying script performs exact symbolic and finite-field checks.
...
\[
 ...
 =-\frac{2e_n}{(n+1)(n+2)}\ne0.
\]
Hence \eqref{eq:basefiber} has degree exactly $n+2$ and has $n+2$ simple roots generically.  This reconstruction is made on the dense open set where the birational chart and its denominators are defined: in particular $\nu,u,g,c\ne0$.  A root $\nu=0$ would force $\alpha=0$ in \eqref{eq:basefiber}, so it creates no extraneous point over a general target.  Each remaining root reconstructs a unique $(s,t)$, and one then uniquely recovers
...
\begin{align*}
 ...
 Q_1\bmod103&=20(\nu+11)(\nu+18)(\nu+21)(\nu^2-50\nu+40).
\end{align*}
Both primes are good: the specialized discriminant is nonzero modulo each, so the reductions are squarefree and the primes are unramified in the splitting field of $Q_1$.  Their Frobenius cycle types show that $\operatorname{Gal}(Q_1/\Q)$ contains a $5$-cycle and a transposition.  Irreducibility modulo $7$ also makes $Q_1$ irreducible over $\Q$, so this group is transitive; transitivity in prime degree implies primitivity, and a primitive subgroup of $S_5$ containing a transposition is $S_5$.  Since this rational target lies off the discriminant and denominator loci, the specialization theorem embeds its Galois group, up to conjugacy, into the generic arithmetic monodromy group.  The latter is a subgroup of $S_5$ containing $S_5$, hence is $S_5$.
...
\[
 R=20\nu^4-80\nu^3+60\nu^2+3\nu+15.
\]
Its exact factorizations have types $(4)$ modulo $11$, $(1,3)$ modulo $7$, and $(1,1,2)$ modulo $19$; the discriminant is nonzero modulo each prime, so these are good, unramified reductions.  Irreducibility modulo $11$ proves that $R$ is irreducible over $\Q$, hence its Galois group is transitive and contains a $4$-cycle.  The reduction modulo $7$ supplies a $3$-cycle.  The group order is therefore divisible by $12$; among transitive subgroups of $S_4$, a subgroup of order divisible by $12$ is $A_4$ or $S_4$, and the $4$-cycle excludes $A_4$.  Thus $\operatorname{Gal}(R/\Q)=S_4$ (the reduction modulo $19$ additionally supplies a transposition).  The target is off the discriminant and denominator loci, so specialization embeds this group, up to conjugacy, into the generic arithmetic monodromy group; consequently that generic group is also $S_4$.

The generic quartic \eqref{eq:quartic} is irreducible over $\C(a,b,c)$: over $K=\C(b,c)$ it is monic and primitive in $K[a][\nu]$, linear in $a$, and its $a$-coefficient $2c^2$ is a unit of $K$.  In any factorization one factor has $a$-degree zero, and comparison of the $a$-coefficient makes it divide that unit.  Thus the geometric group is transitive; it is also normal in the arithmetic $S_4$, so it is $V_4$, $A_4$, or $S_4$.  In the $V_4$ case the degree-four extension would itself be Galois (the transitive $V_4$ action is regular).  Campbell's Galois case over $\C$ says that a polynomial map with nonzero constant Jacobian and Galois induced function-field extension has a polynomial inverse \cite{Campbell73,Campbell13}, contradicting degree four.  To exclude $A_4$, use the three specializations above; their discriminants are
\[
 -54873514800,
...
For completeness, degree two cannot occur.  A separable extension of degree two is Galois, and the classical Galois case over $\C$ \cite{Campbell73,Campbell13} then makes the Keller map invertible, hence of degree one.  This degree-two exclusion is an already-known corollary of the classical theorem and is not claimed as new.
\end{proof}
...
\bibitem{Campbell13}
L. Andrew Campbell,
\emph{On the rational real Jacobian conjecture},
arXiv:1210.0251v4 [math.AG], 2013.
\url{https://arxiv.org/abs/1210.0251}.

\bibitem{Gallagher26}
Alexis Gallagher,
\emph{The Jacobian counterexample, explained},
updated and accessed July 20, 2026.
